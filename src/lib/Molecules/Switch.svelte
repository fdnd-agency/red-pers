<script>
    ...
    const toggleSwitch = async () => {
      isOn = !isOn; // Toggle de state
      dispatch('toggle', { isOn });
  
      if (isOn) {
        // Gebruik View Transitions API om een soepele overgang te creëren
        if (document.startViewTransition) {
          document.startViewTransition(() => {
            // Navigeren naar een nieuwe pagina met SvelteKit
            goto('/Christmas');
          });
        } else {
          // Fallback als View Transitions API niet wordt ondersteund
          goto('/Christmas');
        }
      }
    };
