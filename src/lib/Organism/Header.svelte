<script>
    /** @type {import('./$types').PageData} */

    export let alwaysSticky = true;

    const dateFormat = {
        month: 'long',
        day: 'numeric',
        weekday: 'long',
        year: 'numeric'
    };

    import SearchBar from "$lib/Molecules/SearchBar.svelte";
    import Nieuwsbrief from "$lib/Molecules/Nieuwsbrief.svelte";
    import Donatiebtn from "$lib/Molecules/Donatiebtn.svelte";
    import ChristmasLogo from "$lib/Atoms/ChristmasLogo.svelte";
    import { onMount } from "svelte";
    import Nav from "./Nav.svelte";

    let sticky = alwaysSticky;
    let audioMuted = true;
    let audioElem;
    
    if (!alwaysSticky) {
        onMount(() => {
            // set sticky to true if we have scrolled > 50px
            window.addEventListener("scroll", () => {
                sticky = window.scrollY > 50;
            });
        });
    }

    function toggleMute() {
        audioMuted = !audioMuted;
        audioElem.play();
    }

</script>

<!-- class sticky when sticky is true, class animate when header is not alwaysSticky -->
<header class:sticky={sticky} class:animate={!alwaysSticky}>
    <section class="top">
        <ul>
            <li>Colofon</li>
            <li>Over</li>
            <li>Meedoen</li>
            <li>Contact</li>
        </ul>
    </section>
    <section class="main-header">
        <div class="main-header-inner">
            <div class="date">
                <p class="date-bold uppercase">{(new Date()).toLocaleDateString("nl-NL", dateFormat)}</p>
                <p class="uppercase">Podium voor de journalistiek</p>
            </div>
            <a href="/" class="logo-container">
                <!-- <img src="/RedPers_Logo_Cmyk_Black.webp" alt="RedPers logo" width="280" height="70" /> -->
                <ChristmasLogo/>
            </a>
            <ul>
                <li>
                    <button on:click={toggleMute}>
                        <img class="audio-icon" src="/mute.png" alt="mute" class:hidden={audioMuted}>
                        <img class="audio-icon" src="/unmute.png" alt="unmute" class:hidden={!audioMuted}>
                    </button>
                </li>
                <li><Nieuwsbrief /></li>
                <li><Donatiebtn /></li>
                <li class="search">
                    <SearchBar resultsPage="/search" name="searchterm" placeholder="Zoeken..." />
                </li>
            </ul>
            <audio src="/merry_christmas.mp3" bind:muted={audioMuted} loop bind:this={audioElem} />
        </div>
    </section>
    {#if alwaysSticky}
        <!-- Only include nav in header when header is alwaysSticky -->
        <Nav alwaysSticky={alwaysSticky} />
    {/if}
</header>

<style>
    audio {
        display: none;
    }

    .hidden {
        display: none;
    }
    
    .audio-icon {
        width: 1.5em;
        cursor: pointer;
    }
    
    .main-header {
        background-color: var(--paper-color);
        padding: 3em 0;
        width: 100vw;
        overflow-y: hidden; /* Hide the inner borders when animating */

        padding: 0 0;
        background-color: var(--background-color);

        height: 80px;
    }
    
    .animate .main-header {
        height: 140px;
        padding: 3em 0;
        background-color: var(--paper-color);

        animation: auto linear shrink-header both;

        animation-timeline: view();
        animation-range: 100vh calc(100vh + 40em);
    }
    /* Bron: https://kizu.dev/scroll-driven-animations/ */

    .sticky.animate {
        /* Total height of main-header, required when making position fixed */
        margin-bottom: 230px;
    }

    .main-header-inner {
        display: flex;
        align-items: center;
        height: 100%;
        justify-content: space-between;

        max-width: var(--main-width);
        padding: 0 10px;
        border-top: 5px double var(--text-color);
        border-bottom: 5px double var(--text-color);
        margin: 0 auto;
    }

    .sticky .main-header {
        position: fixed;
        top: -1px; /* Hide upper inner border */
        left: 0;
        border-bottom: 1px solid black;
    }

    .logo-container {
        height: 50%;
    }
    
    img {
        height: 100%;
        object-fit: contain;
    }

    .main-header ul {
        display: flex; 
        list-style: none; 
        margin: 0;
        cursor: pointer;
        padding: 0;
        margin-left: 50px;
        gap: 20px;
        font-family: var(--menu-item);
    }

    .date {
        display: flex;
        flex-direction: column; 
        margin-right: 20px;
    }

    .date p {
        margin: 0;
        font-family:var(--menu-item);
        font-size: 14px;
    }

    .date-bold {
        font-weight: bold;
        text-wrap: nowrap;
    }

    ul {
        list-style-type: none;
        display: flex;
        gap: 15px; 
        padding: 0;
        margin: 0;
    }

    .search {
        --search-bar-width:
    }

    .top li:hover {
        color: #000000;
        background-color: #f0f0f0;
        transition: var(--hover);
    }

    ul li {
        display: flex;
        align-items: center;
    }

    .top {
        background-color: var(--text-color);
        display: flex;
        color: white;
        justify-content: end;
    }
    
    .top ul {
        display: flex;
        list-style: none;
        margin: 0;
        cursor: pointer;
        padding: 0;
    }

    .top li {
        padding: 15px;
        font-family: var(--menu-item);
        font-size: 14px;
    }

    @keyframes shrink-header {
        0% {
            padding: 3em 0;
            background-color: var(--paper-color);
        }
        50% {
            padding: 0 0;
            background-color: var(--background-color);
            height: 140px;
        }
        100% {
            padding: 0 0;
            background-color: var(--background-color);

            height: 80px;
        }
    }
</style>